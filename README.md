...
- **iOS 网页端保存到相册**：在 iPhone/iPad 上点击下载按钮后，弹出全屏视频预览并显示加载进度；加载完成后点击「打开系统分享菜单」，再选择「存储视频」保存到手机相册，不再依赖 Safari 不稳定的长按菜单。
- iOS 设备上按钮文案从「下载」变为「保存相册」，明确操作意图。
- 非 iOS 设备（桌面、Android）保持原有下载逻辑，不受影响。
- 关闭弹窗时自动取消正在进行的 XHR 请求并释放 Blob URL，避免内存泄漏。
...
- 手机浏览器端点击下载按钮会走 `/api/download` 代理下载。
- **iOS Safari 专属流程**：iPhone/iPad 上按钮显示「保存相册」。点击后弹出全屏黑底弹窗，实时显示视频加载进度（MB / 总 MB）；加载完成后点击「打开系统分享菜单」，再选择「存储视频」即可保存到相册。该流程使用浏览器文件分享能力，避免依赖 `blob:` 视频不稳定的长按菜单。
- Android 及桌面浏览器点击「下载」行为不变，直接触发文件下载。
