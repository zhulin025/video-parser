...
- **iOS 网页端保存到相册**：在 iPhone/iPad 上点击下载按钮，不再走浏览器下载到"文件" App，而是弹出全屏视频预览弹窗，实时显示加载进度，加载完成后提示用户长按视频选择「存储到照片」，直接保存到手机相册。
...
- **iOS Safari 专属流程**：iPhone/iPad 上按钮显示「保存相册」。点击后弹出全屏黑底弹窗，实时显示视频加载进度（MB / 总 MB），加载完成后展示视频播放器，并提示长按视频选择「存储到照片」，即可直接保存到手机相册，无需经过“文件” App。
